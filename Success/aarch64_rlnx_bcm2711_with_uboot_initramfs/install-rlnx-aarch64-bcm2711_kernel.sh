...
echo " "; echo "Added Kernel used to $KERNEL_USED"
...
if [ $REGENERATE_ALL ==  1 ]; then
  ...
  if [ ! -d "$THIS_SCRIPT_DIR"/$RASPBERRYPI_LINUXOS_DIRECTORY_NAME ]; then
    ...
    sudo git clone --depth=1 -b rpi-5.19.y $RASPBERRYPI_LINUXOS_GIT_REPO #4.19.y
  ...
  echo " "; echo "Clean files - mrproper"
  sudo make -j$MAKE_CORES mrproper
  ...
  #sudo make -j$MAKE_CORES ARCH=$KERNEL_ARCH CROSS_COMPILE=$CROSS_COMPILE CXXFLAGS="-march=armv8-a+crc -mtune=cortex-a72" CFLAGS="-march=armv8-a+crc -mtune=cortex-a72"
  ...
  echo " "; echo "Now test modules_install"
  sudo make modules_install

  echo " "; echo "Copy Image File to SD card"
  sudo cp -v $THIS_SCRIPT_DIR/$RASPBERRYPI_LINUXOS_DIRECTORY_NAME/arch/$KERNEL_ARCH/boot/$IMAGE_NAME $MOUNT_BOOT_DIRECTORY/$KERNEL_USED.img

  echo " "; echo "Copy dtb Files to SD card"
  sudo cp -v $THIS_SCRIPT_DIR/$RASPBERRYPI_LINUXOS_DIRECTORY_NAME/arch/$KERNEL_ARCH/boot/dts/$PROCESSOR_SOC/$DTB_RPI_FILE $MOUNT_BOOT_DIRECTORY
  ...
  echo "Copy start4.elf fixup4.dat files from raspberry pi firmware boot to SD card mount $MOUNT_BOOT_DIRECTORY"
  sudo cp -v $THIS_SCRIPT_DIR/../$RASPBERRYPI_FIRMWARE_ROOT_DIRECTORY/start4*.elf $MOUNT_BOOT_DIRECTORY
  sudo cp -v $THIS_SCRIPT_DIR/../$RASPBERRYPI_FIRMWARE_ROOT_DIRECTORY/fixup4*.dat $MOUNT_BOOT_DIRECTORY
