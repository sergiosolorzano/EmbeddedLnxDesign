...
if [ $REGENERATE_ALL ==  1 ]; then
  ...
  make -j$MAKE_CORES ARCH=$KERNEL_ARCH CROSS_COMPILE=$CROSS_COMPILE modules

  #Build dtbs
  echo " "; echo "Build dtbs"
  make -j$MAKE_CORES ARCH=$KERNEL_ARCH CROSS_COMPILE=$CROSS_COMPILE dtbs
  ...
  sudo cat <<< 'console=serial0,115200 console=tty1 root=/dev/'$ROOT_PARTITION' rootfstype=ext4 rootwait' >> $THIS_SCRIPT_DIR/$UBOOT_BOOTARGS_FILENAME
