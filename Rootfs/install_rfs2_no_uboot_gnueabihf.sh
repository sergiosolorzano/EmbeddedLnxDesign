...
X_TOOLCHAIN_ARCH_TRIPLET_INSTALLED="/usr/bin/arm-linux-gnueabihf" #/usr/bin/arm-linux-gnueabihf-
...
BUSYBOX_FS_INSTALL_PATH=$THIS_SCRIPT_DIR/$ROOTFS_ROOT_DIR #/home/sergio/EmbeddedLinuxDesign/Kernel/rootfs
USE_INTRAMFS=0
...
echo "crosstoolNG triplet: $X_TOOLCHAIN_ARCH_TRIPLET_INSTALLED"
...
if [ $REGENERATE_ALL ==  1 ]; then
	...
	dirs=($BUSYBOX_FS_INSTALL_PATH/bin $BUSYBOX_FS_INSTALL_PATH/dev $BUSYBOX_FS_INSTALL_PATH/etc $BUSYBOX_FS_INSTALL_PATH/home $BUSYBOX_FS_INSTALL_PATH/lib $BUSYBOX_FS_INSTALL_PATH/proc $BUSYBOX_FS_INSTALL_PATH/sbin $BUSYBOX_FS_INSTALL_PATH/sys $BUSYBOX_FS_INSTALL_PATH/tmp $BUSYBOX_FS_INSTALL_PATH/usr $BUSYBOX_FS_INSTALL_PATH/var)
	for dir in "${dirs[@]}"; do
	  if [ ! -d "/$dir" ]; then
	    mkdir "/$dir"
		else
	...
	make -j$MAKE_CORES ARCH=$KERNEL_ARCH CROSS_COMPILE=$CROSS_COMPILE CONFIG_PREFIX=$BUSYBOX_FS_INSTALL_PATH 2>&1 | tee -a "$BUSYBOX_LOG_FILENAME_PATH/$BUSYBOX_LOG_FILENAME"
	...
	sudo make -j$MAKE_CORES ARCH=$KERNEL_ARCH CROSS_COMPILE=$CROSS_COMPILE CONFIG_PREFIX=$BUSYBOX_FS_INSTALL_PATH install 2>&1 | tee -a "$BUSYBOX_LOG_FILENAME_PATH/$BUSYBOX_LOG_FILENAME"
	
fi

if [ 0 == 1 ]; then
#Get Libraries for the file system
echo " "; echo "Get the sysroot directory path"
...
echo $?
	...
	#no libraries in sysroot for arm-linux-gnueabihf
	echo "No libraries found in arm-linux-gnueabihf/sysroot"
fi
